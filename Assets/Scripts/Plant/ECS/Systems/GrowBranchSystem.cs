...
public class GrowBranchSystem : SystemBase
{
    ...
    {
        EntityQuery query = GetEntityQuery(ComponentType.ReadOnly<GrowBranchTag>(), ComponentType.Exclude<LoadTag>());
        ...
        foreach (Entity empty in entities)
        {
            ...
            entityManager.AddComponentData(newStem, new Translation { Value = new float3(0, height, 0) });
            ...
            entityManager.SetName(newStem, "B" + plantNum);

            entityManager.RemoveComponent(empty, typeof(GrowBranchTag));
        }
